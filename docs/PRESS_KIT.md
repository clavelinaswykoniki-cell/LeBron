...
> 詹黑逻辑拆解器：微信小程序，把评论区黑 LeBron 的话术（"8 分释兵权""Excel 球王""米奇冠军"）拆成结构化反驳卡。150+ 张本地反驳卡，441 条别名匹配。零新依赖，原生小程序 + TDesign。GitHub: github.com/clavelinaswykoniki-cell/LeBron

### 微博（140 字以内）

> 詹黑逻辑拆解器：一个把 LeBron 詹黑话术拆成结构化反驳的微信小程序。输入黑点 → 自动匹配反驳卡 → 一键复制怼回去。150+ 张反驳卡 / 441 别名 / 38 分类。练手项目，GitHub 开源。
...
>
> 数据：150+ 反驳卡 / 441 别名 / 38 分类 / 8 测试 / GitHub Actions CI。
>
